...
internal class SimpleAuthenticationProviderTest {

    @Test
    fun register() {
        Assertions.assertThrows(IllegalArgumentException::class.java) {
            SimpleAuthenticationProvider.getRequired<RefreshTokenCredentials, TokenPrincipal, SimpleRefreshTokenAuthentication>(
                RefreshTokenCredentials::class.java
            )
        }
        val refreshTokenAuthentication = SimpleRefreshTokenAuthentication(mockk())
        SimpleAuthenticationProvider.register(refreshTokenAuthentication)
        assertThat(
            SimpleAuthenticationProvider.getRequired<RefreshTokenCredentials, TokenPrincipal, SimpleRefreshTokenAuthentication>(
                RefreshTokenCredentials::class.java
            ),
