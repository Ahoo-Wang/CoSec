...
- **`token_use`**: Required token purpose. Access tokens use the literal value `access`; refresh tokens use `refresh`
...
The refresh token's `sub` claim is set to the **access token's `jti`**, creating a binding between the two tokens. Verification rejects a token when `token_use` is missing or does not match the operation.

> **Security migration:** Tokens issued before the `token_use` contract was introduced are intentionally rejected. Users must sign in again after upgrading; there is no legacy compatibility mode because it would preserve refresh-token-as-access-token ambiguity.
...
- **`verify(AccessToken)`**: Validates signature, access purpose and expiry, then extracts `TokenPrincipal`
- **`refresh(CompositeToken)`**: Validates refresh signature, purpose and expiry; validates the access token signature and purpose while permitting access expiry; verifies the token binding; then extracts the principal
...
sequenceDiagram
    ...
    Verifier->>Verifier: verify access signature + token_use=access
