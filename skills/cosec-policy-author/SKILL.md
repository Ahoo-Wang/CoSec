...
The Redis types exist only when cache support and `StringRedisTemplate` activate their Spring factories. They use an atomic sliding window, return 429 when exceeded, fail open on Redis errors by default, and fail closed when `strictFailure` is `true`. `MatcherFactoryRegister` initializes local policies after registering these factories, so bootstrap files may use them. The in-memory limiters also return 429 when exceeded.
